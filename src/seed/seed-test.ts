...
async function seedTest() {
  ...
  try {
    ...
    const proposalModel = app.get<Model<ProposalDocument>>(
      getModelToken(Proposal.name),
    );

    async function getOrCreate(
      email: string,
      createData: Parameters<typeof usersService.create>[0],
    ) {
      ...
      const { user: created } = await usersService.create(createData);
    ...
    const regionalId = regional._id.toString();
    ...
    const rep1Id = rep1._id.toString();
    ...
    const supremeGold = allPkgs.find(
      (p) => p.waterType === WaterType.SUPREME && p.name === 'Gold',
    );
    const supremeSilver = allPkgs.find(
      (p) => p.waterType === WaterType.SUPREME && p.name === 'Silver',
    );
    const hwPlatXL = allPkgs.find(
      (p) => p.waterType === WaterType.HOMEWATER && p.name === 'Platinum XL',
    );

    if (!supremeGold || !supremeSilver || !hwPlatXL) {
      throw new Error(
        'Required packages missing — run `npm run seed` first, then retry.',
      );
    }

    const goldId = supremeGold._id.toString();
    const silverId = supremeSilver._id.toString();
    const hwPlatXLId = hwPlatXL._id.toString();
    ...
    const sale1Extras: Partial<CreateSaleDto> = fWithLoan
      ? {
          financier: fWithLoan._id.toString(),
          loanOptionId: (
            fWithLoan.loanOptions[0]._id as Types.ObjectId
          ).toString(),
        }
      : {};
    ...
    console.log(
      `  Sale 1 (Supreme Gold)    rep=$${sale1.commissions.salesRep.toFixed(2)}  recruiter=$${sale1.commissions.directRecruiter}  teamlead=$${sale1.commissions.teamLead}  regional=$${sale1.commissions.regional}  partner=$${sale1.commissions.partner}`,
    );
    console.log(
      `  Sale 2 (Homewater Plat)  rep=$${sale2.commissions.salesRep.toFixed(2)}  recruiter=$${sale2.commissions.directRecruiter}  teamlead=$${sale2.commissions.teamLead}  regional=$${sale2.commissions.regional}  partner=$${sale2.commissions.partner}`,
    );
    console.log(
      `  Sale 3 (Supreme Silver)  rep=$${sale3.commissions.salesRep.toFixed(2)}  recruiter=$${sale3.commissions.directRecruiter}  teamlead=$${sale3.commissions.teamLead}  regional=$${sale3.commissions.regional}  partner=$${sale3.commissions.partner}`,
    );
    ...
    const CUSTOMERS = [
      {
        ...
        notes: 'Interested in whole-home filtration. Has well water.',
      },
      {
        name: 'Sarah Williams',
      ...
      {
        name: 'David Martinez',
        street: '310 Desert Rose Blvd',
        city: 'Tempe',
        state: 'AZ',
        zip: '85281',
        phone: '(480) 555-0234',
        email: 'david.martinez@hotmail.com',
        notes: 'City water. Concerned about chlorine taste.',
      },
      {
        name: 'Emily Thompson',
        street: '55 Saguaro Lane',
        city: 'Mesa',
        state: 'AZ',
        zip: '85201',
        phone: '(480) 555-0310',
      ...
      {
        name: 'Robert Garcia',
        street: '720 Pinnacle Peak Rd',
        city: 'Glendale',
        state: 'AZ',
        zip: '85301',
        phone: '(623) 555-0445',
        email: 'rgarcia@outlook.com',
        notes: 'Family of 5. Hard water issues reported.',
      },
    ];
    ...
    const oldProposalCount = await proposalModel.countDocuments({
      salesRep: rep1._id,
    });
    ...
    const p1 = await proposalsService.create(
      {
        ...
        loanOptionId: fWithLoan
          ? (fWithLoan.loanOptions[0]._id as Types.ObjectId).toString()
          : undefined,
      },
      rep1Id,
      Role.ADMIN,
    ...
    console.log(
      `  created  Proposal for ${cust1.name} — Supreme Gold — $${p1.cashPrice} cash — status: sent`,
    );
    ...
    console.log(
      `  created  Proposal for ${cust2.name} — Supreme Silver — $${p2.cashPrice} cash — status: approved`,
    );
    ...
    console.log(
      `  created  Proposal for ${cust3.name} — Homewater Platinum XL — $${p3.cashPrice} cash — status: draft`,
    );
    ...
    console.log(
      `  created  Proposal for ${cust4.name} — Supreme Gold — $${p4.cashPrice} cash — status: cancel`,
    );
    ...
    console.log(
      `  created  Proposal for ${cust5.name} — Supreme Silver — $${p5.cashPrice} cash — status: converted`,
    );

    console.log('\n=== Done — log in to verify dashboard numbers ===');
    console.log('  rep1@example.com         → sees 3 sales + own commissions');
    console.log(
      '  recruiter@example.com    → sees 3 sales (override on all 3)',
    );
    ...
    console.log(
      '  All roles               → 5 customers + 5 proposals visible to admin',
    );
  } finally {
    await app.close();
  }
